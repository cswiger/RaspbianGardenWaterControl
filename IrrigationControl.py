...
GPIO.output(valve_pin, True)    # immediately turn it off!
GPIO.setup(moisture_pin, GPIO.IN)

# Globals for state
gPostVars = None

# check if a schedule has been saved
try:
  fh = open('schedule.dat','r')
  schedule = eval(fh.read())
  fh.close()
except:
...
def waterCycle(t):
    ...
    gWateringStatus = False
    now = time.time()
    # for watering less than a minute, so no double cycle
    while ( time.time() < now + (60 - duration)):
      updateSched()
      sleep(1)
